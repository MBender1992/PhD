...
dat <- read.csv("Data/Migration_Assay_Boyden_Chamber_20220704.csv")
...
dat$membrane <- factor(dat$membrane, levels = c("all", "bottom"), labels = c("Gesamtanzahl Zellen", "Migrierte Zellen"))





png("Results/Migration Assay/Migration_Assay_raw.png", units="in", width=7, height=2.5, res=600)
dat %>% 
...
png("Results/Migration Assay/Migration_Assay_ratio.png", units="in", width=4, height=3, res=600)
dat_ratio %>% 
...
anova_test(dat_ratio, ratio~Name)
dat_ratio %>% tukey_hsd(ratio~Name)

test <- dat %>% group_by(Name, membrane) %>% summarize(mean = mean(mean)) %>%
  spread(membrane, mean) %>%
  setNames(c("Name", "n", "migrated")) %>%
  mutate(non_migrated = n-migrated) 
...
prop.test(test$migrated[-3], test$n[-3])

# ++ctrl vs TGF
chisq_test(test[-2,3:4])
prop.test(test$migrated[-2], test$n[-2])

# -+ctrl vs TGF
chisq_test(test[-1,3:4])
prop.test(test$migrated[-1], test$n[-1])

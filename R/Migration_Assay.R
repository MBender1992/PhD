...
svg("Results/Migration_Assay_raw.svg", width=6, height=2.5)
dat %>% 
...
svg("Results/Migration_Assay_ratio.svg",  width=4, height=2.5)
dat_ratio %>% 
...
p1<-prop.test(test$migrated[-3], test$n[-3])

# ++ctrl vs TGF
chisq_test(test[-2,3:4])
p2<-prop.test(test$migrated[-2], test$n[-2])

# -+ctrl vs TGF
chisq_test(test[-1,3:4])
p3<-prop.test(test$migrated[-1], test$n[-1])

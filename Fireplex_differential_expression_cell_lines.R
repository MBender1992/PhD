...
two_way_ANOVA <- dat%>% 
  group_by(miRNA) %>%
  anova_test(log_exp ~ Irradiation*cell_line, white.adjust = T) %>%
  adjust_pvalue(method = "fdr") 

# significant cell_line main effect
two_way_ANOVA_cell_line <- two_way_ANOVA %>%
...
pwc_cell_line <- dat%>%
  ...
  filter(p.adj < 0.05 & Irradiation == "control") %>% print(n="all")
